...
- **60-Sec Pitch:** "I built an AI-assisted KYC/AML dashboard that simulates how compliance teams can use LLM summarization and risk scoring to reduce manual review times while maintaining regulatory audit trails."
- **Problem:** Compliance analysts suffer from alert fatigue and spend too much time cross-referencing documents.
- **Decision:** Implemented a single-pane-of-glass view combining AI summaries, submitted docs, and a clear action panel.
- **Tradeoff:** Chose synthetic data over live APIs to ensure privacy and rapid prototyping without relying on paid 3rd-party services.
- **Tech:** React + Tailwind for rapid, scalable UI development.
- **AI:** Framed the AI not as a decision-maker, but as an *assistant* (HITL) to satisfy explainability requirements.
- **Metrics:** Discussed how AHT and Auto-Approval rates define the ROI of this tool.
- **Next Steps:** Connecting to a backend for real-time risk scoring.

## Disclaimer
This is a prototype for portfolio and demonstration purposes only. It does not perform actual identity verification or connect to real sanctions lists. Do not use for production compliance.
